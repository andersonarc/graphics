...
class Client(ip: String, port: Int) {
    ...
    private fun quaternion(data: String): Quaternionf {
        val split = data.split(':')
        return Quaternionf(split[0].toFloat(), split[1].toFloat(), split[2].toFloat(), 0f)
    }
}